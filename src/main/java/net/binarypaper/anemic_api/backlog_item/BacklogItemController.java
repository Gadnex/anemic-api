...
public class BacklogItemController implements BacklogItemAPI {
  ...
  public BacklogItem createBacklogItem(@Valid BacklogItem backlogItem) {
    Product product =
        productRepository
            .findById(backlogItem.getProductId())
            .orElseThrow(
    ...
    if (backlogItem.getSprintId() != null) {
      Sprint sprint =
          sprintRepository
              .findById(backlogItem.getSprintId())
              .orElseThrow(
                  () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid sprint-id"));
  ...
  public BacklogItem updateBacklogItem(UUID backlogItemId, @Valid BacklogItem backlogItem) {
    ...
    if (backlogItem.getSprintId() != null
        && !backlogItem.getSprintId().equals(backlogItemFromDB.getSprintId())) {
      Sprint sprint =
          sprintRepository
              .findById(backlogItem.getSprintId())
              .orElseThrow(
